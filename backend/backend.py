...
def get_routes():
    ...
    try:
        ...
        if not gtfs_data or 'entity' not in gtfs_data:
            return jsonify({"error": "GTFS data unavailable"}), 503
        ...
        return jsonify({"routes": sorted(list(routes))})  # optional sorting for consistency
...
def get_stops_for_route(route_id):
    """Get a list of stop IDs for the specified route"""
    try:
        gtfs_data = fetch_gtfs_data()
        if not gtfs_data or 'entity' not in gtfs_data:
            return jsonify({"error": "GTFS data unavailable"}), 503
        ...
        if not stop_ids:
            return jsonify({"route_id": route_id, "stops": [], "message": "No stops found for this route"}), 404

        return jsonify({
            "route_id": route_id,
            "stops": sorted(list(stop_ids))  # optional sorting for consistency
        })

    except Exception as e:
        logger.error(f"Error fetching stops for route {route_id}: {str(e)}")
        return jsonify({"error": f"Server error: {str(e)}"}), 500
